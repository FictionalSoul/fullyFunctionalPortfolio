...
const MovingImg = ({ title, img, link }) => {
    ...
    function handleMouse(event) { //Image will move with our cursor moves on Link
    ...
    return (
        <Link href={link} target='_blank'
            ...
            {/* Set z property so that we can see the image */}
        </Link>
    )
}
...
const Article = ({ img, title, date, link }) => {
    return (
        ...
        viewport={{once:true}}
        className='relative w-full p-4 py-6 my-4 rounded-xl flex items-center justify-between text-dark
         bg-light first:mt-0 border border-solid border-dark border-r-4 border-b-4 dark:border-light dark:bg-dark
...
const FeaturedArticle = ({ img, title, time, summary, link }) => {
    return (
        <li className=' relative col-span-1 w-full p-4 bg-light border border-solid
         border-dark rounded-lg dark:bg-dark dark:border-light'>
...
const articles = () => {
    return (
        <>
            <Head>
                <title>Vivek Saboo | Articels Page</title>
                <meta name='description' content='
            A confident young developer with hunger to learn something new each & every day.
             Passionate towards to achieve the dreams and goal that i"ve for myself.'>
                </meta>
            </Head>
            <TransitionEffect />
            <main className='w-full flex flex-col items-center justify-center overflow-hidden dark:text-light'>
                <Layout className='pt-16 '>
                    <AnimatedText text="Words Can Change The World!" className='mb-16 lg:!text-7xl sm:mb-8 
